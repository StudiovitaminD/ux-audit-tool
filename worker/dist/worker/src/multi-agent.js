...
function normalizeFindings(value, pillar, buckets) {
...
export async function runSpecialistReview(env, input) {
    ...
    const outputs = await Promise.all(Object.entries(ROLES).map(async ([name, role]) => {
        try {
            ...
            return [name, { status: "complete", findings: normalizeFindings(parsed.findings, role.pillar, buckets) }];
        }
        catch (error) {
            return [name, { status: "failed", error: error instanceof Error ? error.message : String(error), findings: [] }];
        }
    }));
    const agents = Object.fromEntries(outputs);
    const candidates = outputs.flatMap(([, result]) => result.findings);
    let reviewer = { status: "complete" };
