...
function toSnakeEnum(value) {
    if (typeof value !== "string")
        return value;
    const v = value.trim().toLowerCase();
    if (v === "saas" || v === "saas / platform" || v === "platform" || v === "saas")
        return "saas";
    if (v === "e-commerce" || v === "ecommerce" || v === "e commerce")
        return "ecommerce";
    if (v === "marketing website" || v === "website" || v === "marketing_website")
...
function parseStoredIntake(value) {
    if (typeof value === "string") {
...
async function loadStoredIntake(env, doc) {
    const fromInline = parseStoredIntake(doc.intake_json) ??
    ...
    if (provider === "firestore_document" || provider === "firestore_chunks") {
        try {
            ...
            if (metadata?.intake_json)
                return parseStoredIntake(metadata.intake_json);
...
async function runJob(env, reportId) {
    const db = getFirestore(env);
    ...
    const doc = snap.data() || {};
    const intakeRaw = await loadStoredIntake(env, doc);
    const parsed = IntakeSchema.safeParse(intakeRaw);
    if (!parsed.success)
        throw new Error("Invalid intake payload");
    const intake = { ...parsed.data, product_type: toSnakeEnum(parsed.data.product_type) };
    const buckets = Array.from(new Set((intake.selected_buckets || [])
        .filter(Boolean)
        .map((bucket) => normalizeBucketName(bucket))
        .filter((bucket) => (QUESTION_BANK[bucket] || []).length > 0)));
    if (!buckets.length)
        throw new Error("No supported buckets selected");
    ...
    const narrativeModel = userRole === "free" || planType === "free"
...
const server = http.createServer(async (req, res) => {
    try {
        ...
        if (url.pathname === "/run" && req.method === "POST") {
            ...
            if (!reportId)
                return json(res, 400, { error: "Missing reportId" });
