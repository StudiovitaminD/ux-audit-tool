...
async function uploadScreenshot(env, reportId, name, buf) {
...
export async function collectEvidence(env, reportId, intake) {
    const warnings = [];
        ...
        if (remaining > 0) {
            try {
                ...
                const extra = await p.evaluate(() => {
                    const navLike = Array.from(document.querySelectorAll("nav a[href], header a[href], [role='navigation'] a[href]"));
                    return navLike
                ...
                const abs = extra
                    .map((l) => toAbsUrl(startUrl, String(l.href || "")))
        ...
        for (let i = 0; i < targets.length; i++) {
            ...
            try {
                ...
                const data = await page.evaluate(() => {
                    ...
                    const take = (sel, max) => Array.from(document.querySelectorAll(sel))
                    ...
                    const topNavLinks = navLike
                        ...
                        .filter((x) => x.text && x.href)
                        .slice(0, 12);
                    const ctaLike = Array.from(document.querySelectorAll("a[href], button, [role='button'], input[type='submit'], input[type='button']"));
