import { UX_AUDIT_ANSWER_STATES, UX_AUDIT_BUCKETS, UX_AUDIT_BUCKET_IDS, bucketTitleFromQuestion, } from "./ux-audit-core";
export function formatBucketOption(option) {
    return `${option.label} — ${option.text}`;
}
const QUESTION_OPTIONS = UX_AUDIT_ANSWER_STATES.map((option) => ({
    state: option.state,
    label: option.label,
    score: option.score,
    mark: option.score,
    text: option.state === "pass"
...
function bucketPrefix(bucketName) {
    return UX_AUDIT_BUCKET_IDS[bucketName] || "Q";
}
export const QUESTION_BANK = Object.fromEntries(UX_AUDIT_BUCKETS.map((bucket) => [
    bucket.name,
...
export function normalizeBucketName(bucket) {
    return bucket in QUESTION_BANK ? bucket : bucket.trim();
}
export function getSelectedBucketQuestions(selectedBuckets) {
...
export function bucketQuestionTitle(bucketName, questionId) {
