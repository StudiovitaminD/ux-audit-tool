...
const pillarBuckets = UX_AUDIT_BUCKETS.reduce((acc, bucket) => {
...
export const AUDIT_FRAMEWORK = {
    purpose: "Use this framework to audit digital products consistently, record evidence, score individual criteria, and identify improvement priorities across the three UX pillars.",
    scoringScale: "Score PASS as 1, PARTIAL as 0.5, and FAIL as 0. Exclude NOT_TESTED and N/A from the bucket denominator. Each bucket carries equal weight in the overall score.",
    auditNote: "Accessibility should combine automated checks with manual keyboard and screen-reader testing. Performance should be measured under realistic device and network conditions. If Screen Reader Support or Performance are selected, capture at least one semantic/ARIA inspection and one mobile/responsive pass so the model has enough evidence to give a best-effort score.",
    pillars: Object.entries(pillarBuckets).map(([name, buckets]) => ({
        name,
...
export function buildAuditFrameworkBrief() {
    ...
    for (const item of AUDIT_FRAMEWORK.checklist)
        lines.push(`- ${item}`);
    return lines.join("\n");
}
export function buildBucketFrameworkBrief(bucketName) {
    for (const pillar of AUDIT_FRAMEWORK.pillars) {
