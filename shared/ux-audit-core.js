...
export const UX_AUDIT_BUCKET_IDS = {
    "Visual Feedback": "VF",
    "Color & Contrast": "CC",
...
export const UX_AUDIT_ANSWER_STATES = [
    {
        state: "pass",
        label: "Pass",
        score: 1,
...
export function getAnswerStateRecord(state) {
    return UX_AUDIT_ANSWER_STATES.find((item) => item.state === state) || null;
}
export function answerStateFromValue(value) {
    ...
    if (!normalized)
        return null;
    if (["pass", "p"].includes(normalized))
        return "pass";
    if (["partial", "partially", "partial_score"].includes(normalized))
        return "partial";
    if (["fail", "failed", "no", "0"].includes(normalized))
        return "fail";
    if (["not_tested", "nottested", "untested", "not_verified", "unverified"].includes(normalized)) {
        return "not_tested";
    }
    if (["n_a", "na", "n/a", "not_applicable", "notapplicable"].includes(normalized))
        return "n_a";
    return null;
}
export function bucketTitleFromQuestion(question) {
